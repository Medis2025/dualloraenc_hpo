...
#   mode 映射：
#     - "pipeline" / "all" / 其它默认：先 DualLoRAEnc 召回 top-K，再在这些候选里做 exact/fuzzy
#     - "duallora"（DualLoRAEnc only）：仅 direct_NER_bert，全库排序
#     - "fuzzy"（Fuzzy only）：exact + fuzzy（不使用 direct_NER_bert，全库）
# =============================================================================
...
def stageA_ranked(
    ...
    """
    返回 [(hpo_id, stageA_score)].

    新逻辑（pipeline 模式）：
      1) 先用 DualLoRAEnc（direct_NER_bert）在全库上打分 → 取 top-K 作为候选池
      2) 在这个候选池内，再叠加 exact / fuzzy 分数（若启用），取 max 作为最终得分

    其它模式：
      - "duallora"：只用 direct_NER_bert 在全库上排序
      - "fuzzy"：只用 exact + fuzzy，在全库上排序（与之前相同）
    """
    ...
    # ----- direct_NER_bert -----（DualLoRAEnc span encoder，全库打分）
    ...
    # 纯 DualLoRAEnc 模式：直接按 direct_map 排序返回
    if mode in ("duallora", "ner_only", "direct_only"):
        if not direct_map:
            return []
        scored = sorted(direct_map.items(), key=lambda x: x[1], reverse=True)
        return scored

    # 在 pipeline/all 模式下，若启用 direct，则先用 direct 选出 top-K 候选池
    candidate_ids_set = None  # type: ignore
    if use_direct and direct_map:
        # 这里复用 fuzzy_limit 作为 DualLoRA 召回的 K
        K = max(1, min(fuzzy_limit, len(direct_map)))
        top_direct = sorted(direct_map.items(), key=lambda x: x[1], reverse=True)[:K]
        candidate_ids_set = set(hid for hid, _ in top_direct)
    else:
        # fuzzy-only 模式 或 direct_map 为空：不限制词表
    ...
    if use_exact:
        ...
        # 若有候选池，则 exact 仅在候选池内生效
        if candidate_ids_set is not None:
            exact_map = {hid: s for hid, s in exact_map.items() if hid in candidate_ids_set}
    ...
    if use_fuzzy and HAS_RAPIDFUZZ:
        try:
            ...
            thr = 88.0
            tmp_map: Dict[str, float] = {}
            for cand, rf_score, _ in hits:
                if rf_score < thr:
                    continue
                for hid in ontology.term2ids.get(cand, []):
                    s = (max(rf_score, 0.0) / 100.0) * 0.5
                    prev = tmp_map.get(hid, 0.0)
                    if s > prev:
                        tmp_map[hid] = s

            # 若有候选池，则 fuzzy 仅在候选池内生效
            if candidate_ids_set is not None:
                fuzzy_map = {hid: s for hid, s in tmp_map.items() if hid in candidate_ids_set}
            else:
                fuzzy_map = tmp_map
    ...
    # 若有 DualLoRA 候选池，则 direct_map 也限制在候选池内（pipeline/all）
    if candidate_ids_set is not None and direct_map:
        direct_map = {hid: s for hid, s in direct_map.items() if hid in candidate_ids_set}
